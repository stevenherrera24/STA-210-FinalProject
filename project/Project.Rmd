...
pairs(num ~ winner_seed + winner_ht + winner_age + winner_rank + 
        mean_w_df + mean_w_svpt + mean_w_1stIn + mean_w_1stWon + 
        mean_w_2ndWon + mean_w_SvGms + mean_w_bpSaved + mean_w_bpFaced + 
        mean_minutes, data=grass, pch = 16,
      main = "Matrix of scatterplots for Tournament Wins and Variables")
```
...
```{r}
hardback <- ols_step_backward_aic(hardcourtmodel)
newhardcourtmodel <- lm(num ~ winner_seed + winner_ht + winner_rank + 
        mean_w_df + mean_w_svpt + mean_w_bpSaved, data=hard)
tidy(newhardcourtmodel)
```

```{r}
clayback <- ols_step_backward_aic(claycourtmodel)
newclaycourtmodel <- lm(num ~ winner_seed + winner_ht + winner_rank + mean_w_svpt + mean_w_bpSaved, data=clay)
tidy(newclaycourtmodel)
```
```{r}
anova(newhardcourtmodel,newclaycourtmodel)
```

```{r}
grassback <- ols_step_backward_aic(grasscourtmodel)
```
