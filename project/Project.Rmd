...
Confirming on the following website: https://en.wikipedia.org/wiki/2016_Apia_International_Sydney_–_Men%27s_Singles we found that Jeremy Chardy did not in fact play a match that last 1,146 minutes. Therefore, we will consider removing the observation.
...
```{r}
ten %>%
  mutate(predicted = predict.glm(final.base.model,type="response"),
         residuals = residuals.glm(final.base.model,type="response"))

binnedplot(x=ten$predicted,y=ten$residuals,xlab="Predicted")
```
