...
```{r}
x0 = data.frame(minutes = 75, ht = 188, rankpoints = 11689 , ace =2 , df = 0, bpSaved = 2, surface = "Hard")
predict.glm(final.base.model,x0,interval="response")
...
newdata <- ten %>%
  filter(tourney_name == "Monte Carlo Masters",
         winner_name == "Roger Federer")
predict.glm(final.base.model, newdata=newdata, type="response")
...
djo <- data.frame(minutes = 75, ht = 188, rankpoints = 11680, ace = 2, df = 0, bpSaved = 2, surface = "Hard")
predict.glm(final.base.model, newdata=fed, type="response")
predict.glm(final.base.model, newdata=djo, type="response")
```
...
ddd <- ten %>%
  filter(name == "Roger Federer",
         winner_name == "Roger Federer")
ddd
predict.glm(final.base.model, newdata=ddd, type="response")
